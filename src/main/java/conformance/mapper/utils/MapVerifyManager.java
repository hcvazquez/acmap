...
public class MapVerifyManager {
	...
	public static int isCorrectMappingInt(String n1, String n2) {
		...
		//	System.out.println("MAP CURATED: "+n1+" "+n2);
			return -1;
		}
...
//		System.out.println("F-Measure: "+fmeasure);
	...
	public String getSolutionsCluster2(
		...
		for(int i=0;i<alignment.size();i++){
			...
			for(PairOE pair:pairs){
				boolean correct = MapVerifyManager.getInstance().isCorrectMapping(pair.getElement2().getShortName(), pair.getElement1().getName());
				//System.out.println(pair.getKey()+" -> "+DataSetManager.getInstance().getInstanceMap().get(pair).toString()+" "+correct);
				if(correct){
					truePositiveVar++;
				}else{
